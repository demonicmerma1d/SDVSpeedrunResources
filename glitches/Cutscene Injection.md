...
## Mechanism
Similar to other injection glitches, cutscene injection operates by inserting valid command inputs into the player name, which when the game replaces the player name into a cutscene(event) script inside a dialogue, it proceeds to attempt to execute the injected commands. The cutscenes which are useful for this are both the mines and hospital death cutscenes.
...
Deadly friendship involves naming the player `/friendship Alex 99 .` and `/friendship Maru 99 .` respectively for Alex and Maru (no other marriageable npcs have short enough names to fit within the line limit), then chain dying in the mines to repeatedly trigger the mines death cutscene, which says the player name.

### Dialogue Extension
In order to maintain valid syntax over newlines, you have to smuggle the newlines either inside the `speak` command, which is how dialogue is implemented in cutscenes, or as the whitespace character between command arguments, since this gets parsed out on execution. Both of these can be seen for example in the below paste, which is the paste for (1.2) glitched Harvey marriage.(credit pending)
...
- `Friendship <name> <number>` - This attempts to add a given friendship amount `<number>` to `<name>` if they are in the friendshipData, which requires the npc to be met by the player. This number can be negative
- `end` - This is the generic cutscene ending command, with multiple previously useful variants in speedrun routes.
...
- `skippable` - Lets you skip the remaining portion of the cutscene.
...
- `mail <letter ID>` - Adds a letter to the mail for tomorow.
- `money <amount>` - Adds `amount` of money to the player, this number can be negative.
