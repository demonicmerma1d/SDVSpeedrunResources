...
def formatFile(filePath,optimize = True,gender = True,noFormat = False, width = 171, marnieWidth = 156,verboseFormat = True, justPrint = True):
    ...
    if '///PASTE' not in text:
        text = '///PASTE' + text
    result = []
    for item in text.split('///PASTE')[1:]:
        upItem = item.upper()
        if '///MARNIE' in upItem:
            outWidth = marnieWidth
        else:
            outWidth = width
        if '///BOY' in upItem:
            outGender = False
            ...
            outGender = True
        elif '///OPPOSITE' in upItem:
            outGender = not gender
        else:
            outGender = gender
        if justPrint:
            if '///NAME ' in item:
                print(item.split('///NAME ')[1].split('\n')[0])
            print(formatText(item,optimize,outGender,noFormat,outWidth,verboseFormat))
        else:
            result.append(formatText(item,optimize,outGender,noFormat,outWidth,verboseFormat))
    if not justPrint:
        if len(result) == 1:
            return(result[0])
        return(result)
